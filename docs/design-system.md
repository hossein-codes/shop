...
> **نسخه:** ۲.۶ — ✅ **تأیید نهایی کارفرما (۲۰۲۶-۰۸-۲۲)** · مسیر A+B · فونت: ایران‌سنس · پالت چهارفصل · **تابع از منشور ۱۲ قانونی**  
...
**رنگ اصلی برند:** مرکب `#141414` (آنتراسیتِ گرم) — مثل زارا، رنگِ برند ما مشکیِ خالص نیست؛ آنتراسیت گرم است که هم لوکس است هم کم‌خشمکی. **رنگ لهجه برند:** شنی `#C8B49A` (امضای گرمای ایرانی برند).

| گروه | توکن | هگز | نقش |
|---|---|---|---|
| مبانی | `bg` | `#FAFAF8` | پس‌زمینه اصلی تمام صفحات |
| | `surface` | `#FFFFFF` | سطح کارت‌ها، مودال، درِاور |
| | `surface-alt` | `#F3F0EA` | بلوک‌های ثانویه (سکشن مجله، پس‌زمینه بج) |
| | `canvas-dark` | `#1A1917` | هدر فوتر، بلوک‌های ادیتوریال تیره، فوتر |
| متن | `ink` | `#141414` | متن اصلی، تیترها |
| | `ink-2` | `#5C5A56` | متن ثانویه (توضیحات، لِیبل) |
| | `ink-3` | `#8A8781` | متا و جانشین (زمان، placeholder) — **نه برای اطلاعات خرید** |
| خطوط | `line` | `#E6E2DB` | حاشیه کارت، جداکننده، خط تب غیرفعال |
| | `line-strong` | `#D4CFC6` | حاشیه input در حالت عادی |
| برند | `brand` | `#141414` | رنگ اصلی برند = رنگ CTA اولیه |
| | `accent` | `#C8B49A` | لهجه برند (سطح، هایلایت تزئینی) |
| | `accent-deep` | `#A98F6F` | لهجه قوی (زیرخط لینک hover، بج سردبیری) |
| | `star` | `#B08A3E` | ستاره‌های امتیاز (پُر) |
| معنایی | `brick` | `#B4433A` | تخفیف قیمت **و** پیام خطا |
| | `pine` | `#2F7A4C` | موفقیت (پرداخت موفق، موجودی) |
| | `ochre` | `#9A6B2F` | هشدار (موجودی کم، نکته مهم) |
| | `slate` | `#3F5B6B` | اطلاعات خنثی (وضعیت سفارش) |
| تینت نرم | `brick-soft` | `#F7E9E7` | پس‌زمینه برچسب تخفیف / پیام خطا |
| | `pine-soft` | `#E8F3EC` | پس‌زمینه پیام موفقیت |
| | `ochre-soft` | `#F7EFDF` | پس‌زمینه هشدار ملایم |
| | `slate-soft` | `#E9EFF3` | پس‌زمینه اطلاع‌رسانی |
| حالت CTA | `brand-hover` | `#2E2C29` | هاور دکمه اولیه |
| | `brand-active` | `#000000` | حالت فشرده دکمه |
| | `disabled-bg` | `#E6E2DB` / متن `#8A8781` | دکمه/ورودی غیرفعال |

### ۲.۲ نقشه‌ی استفاده: رنگ برای چه کجاست؟

| نیاز | رنگ | قانون |
|---|---|---|
| **پس‌زمینه صفحه** | صدفی `#FAFAF8` | پیش‌فرض همه صفحات؛ سفید خالص ممنوع (خشک است) |
| **پس‌زمینه کارت/مودال** | سفید `#FFFFFF` | برای «برجسته‌شدن از صفحه»؛ سایه خیلی ملایم |
| **متن بدنه و تیتر** | مرکب `#141414` | پیش‌فرض مطلق |
| **CTA اصلی** | دکمه مرکب با متن صدفی | فقط یک CTA اولیه در هر نما؛ هرگز برای لینک عادی |
| **CTA ثانویه** | شفاف + حاشیه ۱.۵px مرکب + متن مرکب | کنار CTA اصلی (مثلاً «افزودن» + «راهنمای سایز») |
| **قیمت** | مرکب، SemiBold | قیمت هرگز رنگی نیست |
| **تخفیف** | آجری: درصد + قیمت خط‌خورته + بج `brick-soft`/`brick` | تنها قرمزِ مجاز در صفحه؛ فقط روی قیمت — هرگز دکمه، هرگز تیتر |
| **موفقیت** | کاج (متن) + `pine-soft` (سطح) | Toast، پیام پرداخت، وضعیت «تحویل شد» |
| **خطا** | آجری (متن) + `brick-soft` (سطح) | خطای فرم، خطای پرداخت؛ همیشه آیکون + متن، نه فقط رنگ |
| **هشدار** | اخرایی + `ochre-soft` | «تنها ۲ عدد ماند»، نکته مرجوعی |
| **اطلاعات** | لایی + `slate-soft` | وضعیت‌های خنثی سفارش |
| **لهجه برند (شنی)** | سطح‌ها و المان تزئینی | بج «انتخاب سردبیر»، زیرخطِ لینک در hover، نوار تزئینی سکشن — **متن روی آن همیشه مرکب**؛ شنی به‌عنوان رنگ متن ⛔ ممنوع (کنتراست ۲:۱) |
| **تیره** | `canvas-dark` | فقط فوتر و بلوک ادیتوریال حداکثر یکی در هر صفحه؛ متن روی آن صدفی |

### ۲.۳ قاعده ۶۰/۳۰/۱۰ و هماهنگی با عکس

- **۶۰٪** سطوح خنثی (صدفی/سفید/کتان) · **۳۰٪** مرکب (متن + عکس) · **۱۰٪** لهجه (شنی + آجریِ به‌ندرت).
- پالت UI عمداً خنثی است تا **رنگِ خود لباس‌ها** در تصاویر بدرخشد — این تفاوت اصلی ما با فروشگاه‌های رنگی است.
...
|---|---|---|
| مرکب روی صدفی | ۱۷:۱ | ✅ AAA — پیش‌فرض |
| مرکب روی کتان/تینت‌های نرم | ۱۴–۱۶:۱ | ✅ AAA |
| صدفی روی مرکب | ۱۷:۱ | ✅ AAA — دکمه CTA |
| `ink-2` روی صدفی | ۶.۵:۱ | ✅ AA |
| آجری روی صدفی | ۵.۳:۱ | ✅ AA (قیمت تخفیف، خطا) |
| کاج روی صدفی | ۵.۱:۱ | ✅ AA |
| لایی روی صدفی | ۷.۲:۱ | ✅ AA |
| اخرایی روی صدفی | ۴.۶:۱ | ✅ AA فقط با وزن ≥۵۰۰ و اندازه ≥۱۳px |
| مرکب روی شنی | ۹:۱ | ✅ AAA |
| `ink-3` روی صدفی | ۳.۴:۱ | ⚠️ فقط متا/آیکون غیرکلیدی، نه اطلاعات خرید |
| شنی روی صدفی (متن شنی) | ۲:۱ | ⛔ ممنوع مطلق |
| سفید روی شنی | ۲.۲:۱ | ⛔ ممنوع مطلق |
| آجری روی شنی | ۲.۶:۱ | ⛔ ممنوع |
...
- پایه‌ی ما **خنثیِ گرم** است (صدفی/سفید/کتان/آنتراسیت). خنثی‌ها رنگِ رقیب ندارند — نه با پالتوی سرمه‌ایِ زمستان می‌جنگند نه با پیراهن مرغانیِ تابستان؛ فقط بوم‌اند و رنگِ خودِ لباس قهرمان صفحه می‌ماند.
- شنی فقط ~۱۰٪ سطح را می‌گیرد و «خاکی» است — خاکی در هیچ فصلی بیگانه نیست (زمستان = توتیه و کرم، تابستان = شن ساحل). هرگز «بهاری» یا «پاییزی» خوانده نمی‌شود.
...
|---|---|---|
| `radius-sm` | ۲px | بج‌ها |
| `radius-md` | ۴px | دکمه، input، کارت محصول (**امضای لوکسِ سیستم: گوشه‌های تقریباً تیز**) |
| `radius-lg` | ۸px | مودال، bottom-sheet |
| `radius-full` | ۹۹۹px | چیپ فیلتر، سواچ رنگ، آواتار |
| `shadow-sm` | `0 1px 2px rgba(20,20,20,.05)` | کارت‌ها در حالت عادی |
| `shadow-md` | `0 8px 24px rgba(20,20,20,.08)` | کارت hover، دراپ‌داون |
| `shadow-lg` | `0 16px 48px rgba(20,20,20,.16)` | مودال، درِاور |
| لایه‌ها | `0` محتوا · `10` چسبان · `20` هدر · `30` دراپ‌داون · `40` درِاور · `50` مودال · `60` Toast · `70` تولتیپ | ترتیب ثابت؛ هیچ z-index دلخواهی ⛔ |

سایه‌ها همیشه با ته‌رنگ **گرم-مشکی** (نه خاکستری سرد) — هماهنگ با آنتراسیت برند.
...
> قرارداد مشترک همه کامپوننت‌ها: RTL · فونت ایران‌سنس · ارتفاع لمسی ≥۴۴px موبایل · حالت `focus-visible` (حلقه ۲px مرکب با فاصله ۲px) روی همه · عدم استفاده از رنگ به‌تنهایی برای انتقال معنا.
...
| نوع | ظاهر | معنا |
|---|---|---|
| جدید | متن صدفی روی مرکب | ثبت‌شده در ۱۴ روز اخیر |
| تخفیف | متن آجری روی `brick-soft` | فقط روی قیمت |
| پرفروش | متن مرکب روی `accent` (شنی) | امضای گرم برند |
...
- **مینی‌کارت همیشه «نوار پیشرفت ارسال رایگان» را نشان می‌دهد** («تا ارسال رایگان ۴۵۰٬۰۰۰ تومان ماند» + نوار پرشونده با لهجه شنی) — رفع اضطراب هزینه پنهان و افزایش میانگین سبد (نتیجه سلف-اودیت ۱.۵).
...
- **PDP:** سواچ دایره ۴۰px (ناحیه لمس ۴۴) در یک ردیف اسکرول‌شونده؛ انتخاب = حلقه ۲px مرکب با فاصله ۲px؛ نام رنگ جلوی لیبل («رنگ: صدفی»).
...
- همه جفت‌های مجاز/ممنوع در بخش ۲.۴ — خلاصه قوانین: متن بدنه فقط مرکب/`ink-2` روی سطوح روشن · شنی هرگز متن نیست · اخرایی فقط ≥۱۳px وزن ۵۰۰+ · اجزای UI (آیکون، حاشیه معن‌دار) حداقل ۳:۱.
...
:root {
  /* رنگ */
  --bg:#FAFAF8; --surface:#FFFFFF; --surface-alt:#F3F0EA; --canvas-dark:#1A1917;
  --ink:#141414; --ink-2:#5C5A56; --ink-3:#8A8781;
  --line:#E6E2DB; --line-strong:#D4CFC6;
  --accent:#C8B49A; --accent-deep:#A98F6F; --star:#B08A3E;
  --brick:#B4433A; --pine:#2F7A4C; --ochre:#9A6B2F; --slate:#3F5B6B;
  --brick-soft:#F7E9E7; --pine-soft:#E8F3EC; --ochre-soft:#F7EFDF; --slate-soft:#E9EFF3;
  ...
  --sh-md:0 8px 24px rgba(20,20,20,.08);
  --sh-lg:0 16px 48px rgba(20,20,20,.16);
...
- **v2.6 — ۲۰۲۶-۰۸-۲۲ — تعادل تراکم صفحه اصلی (ممیزی صادقانه):** گرید کارت ۴تایی دسکتاپ / ۲.۲تایی موبایل (به‌جای ۳تایی/تمام‌عرض) — اسکن و مقایسه بازگشت · اطلاعات کارت فشرده‌تر (p-4/5، نام ۱۵.۵، کد ۱۰px، قیمت ۱۹px) · دکمه کارت secondary (لبه‌دار) به‌جای توپر — پایان «دیوار دکمه‌های مشکی» · کارت از ~۷۰۰px به ~۵۳۰px (دسکتاپ) و از تمام‌عرض به ~۳۹۰px (موبایل).

- **v2.5 — ۲۰۲۶-۰۸-۲۲ (بازخورد کارفرما — کارت v4.1):** قیمت ستونی راست‌چین (قیمت اصلی بالا · قدیم خط‌خورته + چیپ درصد زیرش — PriceTag با حالت stacked) · حذف پنل «افزودن سریع» با سایز از هاور (دکمه‌ی همیشه‌نمایان باقی ماند، سایز پیش‌فرض می‌افزاید) · حذف ردیف اعتماد از کارت · نقاط تصویر و قرص رنگ‌ها دیگر در هاور محو نمی‌شوند.

- **v2.4 — ۲۰۲۶-۰۸-۲۲ — کارت محصول v4 (spec تصویری کارفرما):** کانتینر سفید + حاشیه line + شعاع ۲۴px + سایه 0 8px 30px (هاور: lift + سایه ۲۵۰ms) · تصویر 5:6 · نقاط تعداد تصویر پایین وسط (محو در هاور) · قرص رنگ‌ها روی عکس پایین-ابتدا · بج توپر (تخفیف آجری/جدید شنی/پرفروش مرکب) · کد محصول + امتیاز (۱۲۳ نظر) + قیمت xl درشت با قدیم کم‌رنگ خط‌خورته · دکمه «افزودن به سبد خرید» همیشه‌نمایان تمام‌عرض · گرید: موبایل تمام‌عرض، دسکتاپ ۳تایی · Mapping رنگ مرجع: #006B9F→آجری، #72C6E8→شنی، آبی-خاکستری‌ها→ink/ink-3 (پالت چهارفصل حفظ شد).
